...
static const CGFloat kWidthOfButtonMargin = 9.0f;
...
- (void)initValues
{
    ...
    _rootScrollView.backgroundColor = [UIColor themeNavBackColor];
...
- (void)buildUI
{
    ...
    QCSlideSwitchDto   * slideSwitchDto = nil;
    for (int i=0; i<number; i++) {
        slideSwitchDto = [self.slideSwitchViewDelegate slideSwitchView:self viewOfTab:i];
        [_viewArray addObject:slideSwitchDto];
        
        [_rootScrollView addSubview:slideSwitchDto.view];
    }
...
{    
    ...
    _shadowImage = [UIImage imageNamed:@"btnBottom.png"];
    [_shadowImageView setImage:_shadowImage];
    [_topScrollView addSubview:_shadowImageView];
    
    QCSlideSwitchDto * slideSwitchDto = nil;
    //顶部tabbar的总长度
    CGFloat topScrollViewContentWidth = _isShowAllTab ? self.frame.size.width-kWidthOfButtonMargin*2 : kWidthOfButtonMargin;
//    CGFloat topScrollViewContentWidth = self.frame.size.width;
    
    //每个tab偏移量
    CGFloat xOffset = kWidthOfButtonMargin;
    for (int i = 0; i < [_viewArray count]; i++) {
        slideSwitchDto   = _viewArray[i];
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        
        if(_isShowAllTab){
        
            xOffset = [self getTabBtnX:i topAllw:topScrollViewContentWidth curX:xOffset];
...
//            xOffset += (topScrollViewContentWidth/[_viewArray count]);
            ...
            CGSize textSize  = [slideSwitchDto.title sizeWithFont:[UIFont systemFontOfSize:kFontSizeOfTabButton]
                                                constrainedToSize:CGSizeMake(_topScrollView.bounds.size.width, kHeightOfTopScrollView)
                                                    lineBreakMode:NSLineBreakByTruncatingTail];
            
//            CGSize textSize  = [slideSwitchDto.title boundingRectWithSize:CGSizeMake(_topScrollView.bounds.size.width, kHeightOfTopScrollView) options:NSLineBreakByWordWrapping attributes:nil context:nil].size;
            //累计每个tab文字的长度
        ...
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, 0, button.frame.size.width, _shadowImage.size.height);
            button.selected = YES;
        }
        [button setTitle:slideSwitchDto.title forState:UIControlStateNormal];
...
//        [button setTitleColor:self.tabItemSelectedColor forState:UIControlStateSelected];
        [button setTitleColor:[UIColor themeSelGrayBorderColor] forState:UIControlStateNormal];
        [button setTitleColor:[UIColor themeNavWordColor] forState:UIControlStateSelected];
 ...
 *  @return
 */
- (CGFloat)getTabBtnX:(NSInteger)index topAllw:(CGFloat)allw curX:(CGFloat)x{
    if(index > 0) {
        x += (allw / [_viewArray count]);
    }
    return x;
}
 ...
 */
- (void)addTabSplite {
    UILabel * label = [[UILabel alloc] initWithFrame:CGRectMake(0, _topScrollView.frame.size.height-0.5f, _topScrollView.frame.size.width, 0.5f)];
    label.backgroundColor = [UIColor themeLineColor];
...
{
    if(_slideSwitchViewDelegate && [_slideSwitchViewDelegate respondsToSelector:@selector(slideSwitchWillBeginScroll)]) {
        [_slideSwitchViewDelegate slideSwitchWillBeginScroll];
    }

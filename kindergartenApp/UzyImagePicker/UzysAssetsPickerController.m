...
- (void)setupOneMediaTypeSelection
{
    if(_maximumNumberOfSelectionMedia > 0)
    {
        ...
        if(_maximumNumberOfSelection > 1)
            self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"选择影片", @"UzysAssetsPickerController", nil);
        else
            self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"选择一个影片", @"UzysAssetsPickerController", nil);
    ...
    else
    {
        if(_maximumNumberOfSelectionPhoto == 0)
        {
            ...
            self.labelSelectedMedia.hidden = NO;
            if(_maximumNumberOfSelection > 1)
                self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"选择音频", @"UzysAssetsPickerController", nil);
            else
                self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"选择一个音频", @"UzysAssetsPickerController", nil);
        }
        else if(_maximumNumberOfSelectionVideo == 0)
        {
            //            self.assetsFilter = [ALAssetsFilter allPhotos];
            ...
            if(_maximumNumberOfSelection >1)
                ...
                self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"选择一张图片", @"UzysAssetsPickerController", nil);
        }
        else
        {
...
- (void)setupGroup:(voidBlock)endblock withSetupAsset:(BOOL)doSetupAsset
{
    ...
    ALAssetsLibraryAccessFailureBlock failureBlock = ^(NSError *error) {
        ...
        [strongSelf setTitle:NSLocalizedStringFromTable(@"没有相册权限!请在设置中检查iphone设置!", @"UzysAssetsPickerController",nil)];
...
- (void)initNoAssetView
{
    ...
    title.text              = NSLocalizedStringFromTable(@"没有图片或者影片", @"UzysAssetsPickerController", nil);
    ...
    message.text            = NSLocalizedStringFromTable(@"你可以使用iTunes同步您的相片", @"UzysAssetsPickerController",nil);
...
- (void)showNotAllowed
{
    ...
    title.text              = NSLocalizedStringFromTable(@"应用没有被允许使用照片或者视频，请在iphone设置中检查是否开启该功能!", @"UzysAssetsPickerController",nil);
    title.font              = [UIFont boldSystemFontOfSize:17.0];
    title.textColor         = [UIColor colorWithRed:129.0/255.0 green:136.0/255.0 blue:148.0/255.0 alpha:1];
    title.textAlignment     = NSTextAlignmentCenter;
    title.numberOfLines     = 5;
    
    message.text            = NSLocalizedStringFromTable(@"您可以在iphone 设置-隐私 中打开开关", @"UzysAssetsPickerController",nil);
...
- (void)showNoAssetsIfNeeded
{
    ...
    voidBlock setNoImage = ^{
        ...
        msg.text = NSLocalizedStringFromTable(@"你可以使用iTunes同步您的相片",@"UzysAssetsPickerController", nil);
    };
    voidBlock setNoVideo = ^{
        ...
        UILabel *msg = (UILabel *)[weakSelf.noAssetView viewWithTag:kTagNoAssetViewMsgLabel];
        msg.text = NSLocalizedStringFromTable(@"你可以使用iTunes同步您的视频",@"UzysAssetsPickerController", nil);
...
- (IBAction)btnAction:(id)sender {
    ...
    switch (btn.tag) {
        ...
        {
            if (![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
                NSString *title = NSLocalizedStringFromTable(@"错误!", @"UzysAssetsPickerController", nil);
                NSString *message = NSLocalizedStringFromTable(@"该设备没有相机", @"UzysAssetsPickerController", nil);
